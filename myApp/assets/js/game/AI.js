...
export class AI extends Player
{
	...
	predictBallPosition(ball)
	{
		...
		if (!this.lastUpdate || now - this.lastUpdate > 1000) 
		{
			...
			// Update AI paddle movement based on predicted position
			this.lastKnownBallY = predictedY; // AI stores the predicted Y position
			this.lastUpdate = now; // Store the current time as the last update time
	...
	update()
	{
		...
		// Ensure AI paddle stays within the canvas bounds
		if (this.paddleY < 0) 
			this.paddleY = 0;
	...
	if (ballSpeed > 5 && player.paddleHits % 3 == 0) // If ball speed is high (e.g., 8 pixels/frame) and player has hit the ball 3 times
